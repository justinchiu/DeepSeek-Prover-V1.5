...
class SearchProcess(mp.Process):
    ...
    def process_print(self, logs, **kwargs):
        print(
            "Process ID: {:3d}    Problem ID: {}    {}".format(
                self.idx, self._current_prob, logs
            ),
            **kwargs,
        )
    ...
    def run(self):
        while True:
            ...
            for sample, info in sample_generator:
                ...
                request_id = self.scheduler.verifier_submit_request(
                    candidate["statement_proposal"]
                )
                request_id_list.append(request_id)
            sample_timecost = time.time() - sample_start_time

            verification_start_wait_time = time.time()
            result_list = self.scheduler.verifier_get_all_request_outputs(
                request_id_list
            )
            ...
            self.process_print(
                "Success: {} / {}    Generation: {:.2f} secs    Verfication: {:.2f} secs".format(
                    success_count,
                    len(candidate_list),
                    sample_timecost,
                    verification_timecost,
                )
            )

            summary_dict = dict(success=[], failure=[])
            for _idx, (candidate, result, info) in enumerate(
                zip(candidate_list, result_list, info_list)
            ):
                success_flag = "success" if result["complete"] else "failure"
                summary_dict[success_flag].append(
                    dict(
            ...
            for success_flag, summary_list in summary_dict.items():
                if len(summary_list) > 0:
                    with open(
                        prob_log_basedir
                        / f"{success_flag}-{log_tag}-{get_datetime()}.pkl",
                        "wb",
                    ) as pkl_f:
