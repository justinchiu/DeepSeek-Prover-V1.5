...
class TreeNode(object):
    ...
    def to_node_list(self):
        return sum(
            [child.to_node_list() for _, child in self.children.items()], start=[self]
        )
    ...
    def to_dict(self):
        return dict(
            info=self._info,
            children={
                edge: child_node.to_dict() for edge, child_node in self.children.items()
            },
        )
    ...
    def update_reward(self, reward, gamma, first_node=True):
        if first_node:
            self._info["_discounted_rewards"] = (
                self._info["_discounted_rewards"] * gamma + reward
            )
            self._info["_discounted_visitation"] = (
                self._info["_discounted_visitation"] * gamma + 1.0
            )
        self._info["_subtree_discounted_rewards"] = (
            self._info["_subtree_discounted_rewards"] * gamma + reward
        )
        self._info["_subtree_discounted_visitation"] = (
            self._info["_subtree_discounted_visitation"] * gamma + 1.0
        )
    ...
    def _update_value(self, gamma):
        discounted_rewards = self._info["_discounted_rewards"] * (
            gamma**self._num_running_jobs
        )
        discounted_visitation = self._info["_discounted_visitation"] * (
            gamma**self._num_running_jobs
        ...
        subtree_discounted_rewards = self._info["_subtree_discounted_rewards"] * (
            gamma**self._subtree_num_running_jobs
        )
        subtree_discounted_visitation = self._info["_subtree_discounted_visitation"] * (
            gamma**self._subtree_num_running_jobs
        ) + (1.0 - (gamma**self._subtree_num_running_jobs)) / (1.0 - gamma)
        self.subtree_value = subtree_discounted_rewards / max(
            subtree_discounted_visitation, 1e-2
        )
...
class RMaxTS(SamplingAlgorithmBase):
    ...
    def _tree_setup(self, data):
        ...
        if ckpt_info is not None:
            ...
            root = self.node_cls(
                code=dict(tactic_code=str(), state_comment=str()), depth=0
            )
            sample_count = 0
            yield_cache = []
    ...
    def _tree_update(self, proof):
        ...
        for info in segments:
            ...
            code = (
                partial_code + info.state_comment
            ...
            if self._encode_length(code) < self.max_tokens:
                if info.goal != prev_goal:
                    node_walk = self._tree_step(
                        ...
                        edge=info.goal,
                        code=dict(
                            tactic_code=partial_code, state_comment=info.state_comment
                        ),
                    )
                    prev_goal = info.goal
        return node_walk
    ...
    def _select_node(self):
        ...
        while len(node.children) > 0:
            ...
            total_visitation = node.visitation + np.sum(
                [child.subtree_visitation for _, child in node.children.items()]
            )

            def hoeffding_ucb(node_visitation):
                return math.sqrt(
                    2.0
                    * math.log(max(total_visitation, 2))
                    / max(node_visitation, 1e-2)
                )
            ...
            for _, child in node.children.items():
                choice_list.append(
                    (
                        child.subtree_value + hoeffding_ucb(child.subtree_visitation),
                        child,
                    )
                )
    ...
    def _rmax_exploration_summarize_results(self, node, proof):
        ...
        # RMax reward
        node.update_reward(
            int(len(self.node_list) > num_nodes_before), gamma=self.gamma
        )
    ...
    def sample(self, data, prob_log_dir, **kwargs):
        ...
        job_slots = [
            ConcurrentJob(self.algorithm_pipeline) for _ in range(self.concurrent_num)
        ]
                ...
                if not job.is_idle():
                    ...
                    if info is not None:
                        ...
                        if info["result"]["complete"]:
                            _proposal, _sample_info = (
                                info["code"],
                                self._post_sample_info(
                                    cost=sample_count,
