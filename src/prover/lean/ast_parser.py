        ...
        if data.get("kind") == "Lean.Parser.Command.docComment":
            ...
            for arg in args:
                if "atom" in arg:
                    ...
                    positions.append(
                        (
                            arg["atom"]["info"]["original"]["pos"],
                            arg["atom"]["info"]["original"]["endPos"],
                        )
                    )
        for value in data.values():
...
def find_kind_name_theorem_lemma_abbrev_def_instance_inductive(file_content, data):
            ...
            if second_node["kind"] == "Lean.Parser.Command.instance":
                ...
                for arg in second_node.get("args", []):
                    if arg.get("node"):
                        if arg.get("node")["args"]:
                            if (
                                arg.get("node")["args"][0].get("node", {}).get("kind")
                                == "Lean.Parser.Command.declId"
                            ):
                                ident = arg.get("node")["args"][0]["node"]["args"][0][
                                    "ident"
                                ]
                                name = ident.get("val")
                                # print(name)
                                name_pos = (
                                    ident.get("info", {}).get("original", {}).get("pos")
                                )
                                # print(name_pos)
                                name_end = (
                                    ident.get("info", {})
                ...
                for arg in second_node.get("args", []):
                    if arg.get("node", {}).get("kind") == "Lean.Parser.Command.declId":
                        ...
                        name_end = (
                            ident.get("info", {}).get("original", {}).get("endPos")
                        )
                        break
    ...
    if kind_pos and kind_end:
        (
            ...
            kind_char_idx_1,
            kind_char_idx_2,
        ) = process_lean_file(file_content, kind_pos, kind_end)
    else:
        (
            kind,
            kind_line_1,
    ...
    if name_pos and name_end:
        (
            ...
            name_char_idx_1,
            name_char_idx_2,
        ) = process_lean_file(file_content, name_pos, name_end)
    else:
        (
            name,
            name_line_1,
...
def find_statement_theorem_lemma_abbrev(file_content, data):
    ...
    for arg in node_1.get("args", []):
        if arg.get("node", {}).get("kind") in [
            ...
            (
                ...
                explicit_char_idx_1,
                explicit_char_idx_2,
            ) = process_lean_file(file_content, start_pos, end_pos)
    ...
    if node_2 is not None and second_node["kind"] in [
...
def find_proof(file_content, data):
    ...
    if positions:
        ...
        (
            ...
            proof_char_idx_1,
            proof_char_idx_2,
        ) = process_lean_file(file_content, proof_start_pos, proof_end_pos)
    else:
...
def process_modifier(file_content, declaration, tactics):
    ...
    if doccomment_vals:
        ...
        (
            ...
            comment_char_idx_1,
            comment_char_idx_2,
        ) = process_lean_file(file_content, comment_start_pos, comment_end_pos)
    else:
        comment = None
    ...
    if attributes_vals:
        ...
        (
            ...
            attributes_line_2,
            attributes_column_2,
            attributes_char_idx_1,
            attributes_char_idx_2,
        ) = process_lean_file(file_content, attributes_start_pos, attributes_end_pos)
    ...
    if pripro_vals:
        ...
        (
            ...
            pripro_char_idx_1,
            pripro_char_idx_2,
        ) = process_lean_file(file_content, pripro_start_pos, pripro_end_pos)
    else:
        pripro = None
    ...
    for tactic in tactics:
        if start_pos <= tactic["pos"] <= end_pos:
            _, A, B, c, d, tactic["pos"], tactic["endPos"] = process_lean_file(
                file_content, tactic["pos"], tactic["endPos"]
            )
...
def theorem_lemma_abbrev(file_content, declaration, tactics):
    (
        ...
        whole_char_idx_1,
        whole_char_idx_2,
    ) = process_modifier(file_content, declaration, tactics)
    ...
    (
        ...
        name_char_idx_1,
        name_char_idx_2,
    ) = find_kind_name_theorem_lemma_abbrev_def_instance_inductive(
        file_content, declaration
    )

    explicitBinder_list, type_list, statement_positions = (
        find_statement_theorem_lemma_abbrev(file_content, declaration)
    )
    positions = extract_positions(declaration)
    positions = [
        (start, end)
    ...
    if statement_positions:
        ...
        (
            ...
            statement_line_2,
            statement_column_2,
            statement_char_idx_1,
            statement_char_idx_2,
        ) = process_lean_file(file_content, statement_start_pos, statement_end_pos)
    else:
    ...
    (
        ...
        proof_char_idx_1,
        proof_char_idx_2,
    ) = find_proof(file_content, declaration)
    declaration_info = {
        "kind": kind,
        "whole_start_pos": {
...
def definition_instance(file_content, declaration, tactics):
    ...
    explicitBinder_list, type_list, statement_positions = find_statement_def(
        file_content, declaration
    )
    positions = extract_positions(declaration)
    positions = [
        ...
        if arg.get("node"):
            if arg.get("node")["args"]:
                if arg.get("node")["args"][0].get("node"):
                    if arg.get("node")["args"][0].get("node")["kind"] in [
                        "Lean.Parser.Term.explicitBinder",
                        "Lean.Parser.Term.implicitBinder",
                        "Lean.Parser.Term.instBinder",
                    ]:
...
def structure(file_content, declaration, tactics):
    ...
    (
        ...
        name_char_idx_1,
        name_char_idx_2,
    ) = find_kind_name_structure(file_content, declaration)

    explicitBinder_list, statement_positions = find_statement_structure(
        file_content, declaration
    )
    positions = extract_positions(declaration)
    positions = [
    ...
    (
        ...
        proof_char_idx_1,
        proof_char_idx_2,
    ) = find_proof_structure(file_content, declaration)
    declaration_info = {
        "kind": kind,
        "whole_start_pos": {
            "position": whole_char_idx_1,
...
def inductive(file_content, declaration, tactics):
    ...
    (
        ...
        proof_char_idx_1,
        proof_char_idx_2,
    ) = find_proof_inductive(file_content, declaration)
    declaration_info = {
        "kind": kind,
        "whole_start_pos": {
            "position": whole_char_idx_1,
...
def lean4_parser(file_content, data):
    ...
    for i, declaration in enumerate(command_asts):
        ...
        if kind in ["Lean.Parser.Command.declaration", "lemma"]:
            if len(declaration.get("node", {}).get("args", [])) > 1 and declaration[
                "node"
            ]["args"][1]["node"]["kind"] in [
                "Lean.Parser.Command.theorem",
                ...
                "node"
            ]["args"][1]["node"]["kind"] in ["Lean.Parser.Command.structure"]:
                declaration_info = structure(file_content, declaration, tactics)
                declarations_info.append(declaration_info)

            elif len(declaration.get("node", {}).get("args", [])) > 1 and declaration[
                "node"
            ]["args"][1]["node"]["kind"] in ["Lean.Parser.Command.inductive"]:
