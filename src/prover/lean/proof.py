...
class Proof(object):
    ...
    def result(self):
        if self._verifier_request_id is not None:
            self._result = self._scheduler.verifier_get_request_outputs(
    ...
    def cleaned_code(self):
        return self.full_code[
            len(self.header) + len(self.formal_statement) : len(self.full_code)
            - len(self.tailer)
        ]
    ...
    def _update_full_code(self, full_code, _result_backup=None):
        ...
        if _result_backup is not None:
            ...
            self._verifier_request_id = self._scheduler.verifier_submit_request(
                dict(
                    code=self.full_code,
                    ast=True,
                    tactics=True,
    ...
    def segmentation(self, result=None):
        ...
        for info in result["sorries"] + result["errors"]:
            ...
            if info_pos >= _prefix_len and not info.get(
                "data", str()
            ).lstrip().startswith("unsolved goals"):
        ...
        for line_idx in range(len(code_lines)):
            if self._line_offset[line_idx] >= _prefix_len:
                ...
                line_lastChar = self._line_offset[
                    line_idx
                ...
                for tactic_info in result["ast"]["tactics"]:
                    ...
                    if (
                        line_lastChar <= endPos
                        and endPos <= line_endPos
                        and pos < pos_min
                    ):
                        pos_min = pos
        ...
        if result["complete"] and (
            len(segments) == 0
            or segments[-1].goal != "no goals"
            or segments[-1].indent != segments[0].indent
        ):
            ...
            segments.append(
                AttrDict(
                    ...
                    state_comment=newline_with_indent.join(
                        [" " * indent_len + "/- tactic state:", "  no goals", "-/\n"]
                    ),
                    goal="no goals",
                    indent=indent_len,
                )
